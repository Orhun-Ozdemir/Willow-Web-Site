...
if start_idx == -1:
    print("Could not find SERP Güncellemesi")
    sys.exit()

# We need to find the exact end of the SEO Live Sync block. 
# It ends right before "if (state.content && target.closest"
end_idx = code.find('      if (state.content && target.closest', start_idx)

if end_idx == -1:
    print("Could not find end of block")
    sys.exit()

replacement = """// SERP Güncellemesi
        if (field === "seoTitle") {
...
# We remove the extra `      }` which were closing the if (target.dataset.seoField) block.
# Actually let's just make sure we close the `if (target.dataset.seoField)` correctly.

new_code = code[:start_idx] + replacement + code[end_idx:]
with open('assets/admin.js', 'w') as f:
    f.write(new_code)
print("Fixed sync block!")
