...
export async function sendLeadNotification(to: string[], lead: LeadMailData) {
  ...
  const from = `"WillowSoft Admin" <${process.env.SMTP_USER}>`;
...
export async function sendTelegramNotification(chatIds: string[], lead: LeadMailData) {
  ...
  const layersArr = Array.isArray(lead.layers)
    ? lead.layers
    : lead.layers ? [lead.layers] : [];
  ...
  // Başlık
  parts.push(isStartProject
    ? `🚀 *YENİ PROJE BAŞLATMA TALEBI*`
    : `📬 *YENİ İLETİŞİM FORMU*`
  );
