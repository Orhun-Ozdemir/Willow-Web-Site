...
import PublishingCenterPanel from "./PublishingCenterPanel";
...
type Tab =
  ...
  | "products" | "news" | "publishing" | "faqs" | "glossary" | "solutions" | "clients" | "company" | "services_page"
  | "special_days"
...
const ICON_PATHS: Record<Tab, string> = {
  ...
  publishing: "M12 19V5m0 0l-5 5m5-5l5 5M5 15v4h14v-4M4 9l8-6 8 6",
  special_days: "M8 2v3m8-3v3M4 9h16M5 4h14a1 1 0 011 1v15a1 1 0 01-1 1H5a1 1 0 01-1-1V5a1 1 0 011-1zm3 9h3v3H8v-3z",
...
const TABS: { section: string; items: { key: Tab; label: string }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "publishing", label: "Yayın Merkezi" },
      { key: "special_days", label: "Özel Günler" },
      { key: "faqs", label: "SSS" },
  ...
  },
  {
    ...
    items: [
      ...
      { key: "storage", label: "Medya & Dosyalar" },
      { key: "history", label: "Değişiklik Geçmişi" },
...
export default function AdminShell() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    ...
    return requested && requested in ICON_PATHS ? requested : "overview";
  });
    ...
    <div className="min-h-screen flex bg-[#f5f3ff]" style={{ fontFamily: "var(--font-body)" }}>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-[#132175] flex flex-col justify-between shrink-0 shadow-xl transform transition-transform duration-200 md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="px-4 py-4 border-t border-white/10 space-y-2">
          ...
          <a
            ...
            className="w-full py-2 bg-white/8 hover:bg-white/15 hover:text-white rounded-lg text-xs font-semibold text-white/70 transition flex items-center justify-center gap-2"
          >
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          ...
          {activeTab === "publishing" && <PublishingCenterPanel />}
          {activeTab === "special_days" && <SpecialDaysPanel />}
